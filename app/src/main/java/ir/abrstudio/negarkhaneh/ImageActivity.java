...
public class ImageActivity extends Activity {
	...
	private class Download implements View.OnClickListener {
		...
		public void onClick(View v) {
			File f = new File(Environment.getExternalStorageDirectory()
					+ File.separator + "negar-" + image.getImageTitle()
					+ ".jpg");
			Bitmap image = ((BitmapDrawable) backgroundImage.getDrawable())
					.getBitmap();
			compressAndSaveImage(f, image);
			addImageToGallery(v.getContext().getContentResolver(), "jpg", f);
			Toast.makeText(v.getContext(), "Saved to Gallery",
	...
	private class Share implements View.OnClickListener {
		...
		@Override
		public void onClick(View v) {
			Toast.makeText(v.getContext(), "sharing...", Toast.LENGTH_SHORT)
					.show();
			Bitmap image = ((BitmapDrawable) backgroundImage.getDrawable())
					.getBitmap();
	...
	private class Like implements View.OnClickListener {
		...
		public void onClick(View v) {
			if (like.getTag() != null) {
				...
				image.Like(getApplicationContext());
	...
	public Uri addImageToGallery(ContentResolver cr, String imgType,
								 File filepath) {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (getIntent() != null && getIntent().getExtras() != null
				&& getIntent().getExtras().containsKey("image")) {
	...
	private void setContents() {
		...
		Glide.with(this).load(image.getImageAddress()).asBitmap().centerCrop()
